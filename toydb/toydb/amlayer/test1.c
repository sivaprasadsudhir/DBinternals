...
main()
{
	system("bash ../data/run.sh 1000000");
	double totalTime = 0;
...
int newValues[SIZE];
	...
	printf("Time\t#Levels\tRootID\t#Nodes\tSpaceAlloc\tSpaceUsed\n");
	printf("%.2lfms\t%d\t%d\t%d\t%d\t\t%d\n",1e3*(totalTime) , iter-1, AM_RootPageNum, allocsDone, PF_PAGE_SIZE*allocsDone, sizeallocated);
